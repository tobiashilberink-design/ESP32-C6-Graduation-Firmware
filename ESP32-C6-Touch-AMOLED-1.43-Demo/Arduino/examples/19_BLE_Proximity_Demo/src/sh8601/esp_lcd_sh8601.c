...
static esp_err_t panel_sh8601_del(esp_lcd_panel_t *panel)
{
    ...
    if (sh8601->reset_gpio_num >= 0) gpio_reset_pin(sh8601->reset_gpio_num);
    free(sh8601);
    return ESP_OK;
}
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_COLMOD, (uint8_t[]){ sh8601->colmod_val }, 1), TAG, "send command failed");
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        case LCD_CMD_MADCTL: is_cmd_overwritten = true; sh8601->madctl_val = ((uint8_t *)init_cmds[i].data)[0]; break;
        case LCD_CMD_COLMOD: is_cmd_overwritten = true; sh8601->colmod_val = ((uint8_t *)init_cmds[i].data)[0]; break;
...
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    ESP_RETURN_ON_ERROR(tx_param(sh8601, sh8601->io, invert_color_data ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
...
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    if (mirror_x) sh8601->madctl_val |= BIT(6);
    else          sh8601->madctl_val &= ~BIT(6);
    if (mirror_y) { ESP_LOGE(TAG, "mirror_y is not supported"); return ESP_ERR_NOT_SUPPORTED; }
    ESP_RETURN_ON_ERROR(tx_param(sh8601, sh8601->io, LCD_CMD_MADCTL, (uint8_t[]){ sh8601->madctl_val }, 1), TAG, "send command failed");
    return ESP_OK;
}
...
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    ESP_RETURN_ON_ERROR(tx_param(sh8601, sh8601->io, on_off ? LCD_CMD_DISPON : LCD_CMD_DISPOFF, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
